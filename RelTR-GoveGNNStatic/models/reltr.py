...
class RelTR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_rel_classes, num_entities, num_triplets, aux_loss=False, matcher=None):
        ...
        self.embeddings.requires_grad = False  # This makes the embeddings static
    ...
    def setup_rgcn(self):
        ...
        # glove_vectors = self.load_glove_embeddings(CLASSES, dim=256)
        
        # # Add padding before and after
        # padding_tensor = torch.zeros(1, 256)
        # padded_embeddings_torch = torch.cat([padding_tensor, glove_vectors, padding_tensor], dim=0)
        
        padded_embeddings_torch = torch.load('/home/iiitd/kirtil/FatigueTraining/RAWDATA/outputs/processed_embeddings_main1.pt')
        
        return padded_embeddings_torch

    def load_glove_embeddings(self, words, dim=256, glove_path=None):
        """
        Load GloVe embeddings for the given words.
        
        Args:
            words: List of words to get embeddings for
            dim: Dimension of GloVe embeddings
            glove_path: Path to GloVe file (if None, uses default location)
        
        Returns:
            Tensor of shape [len(words), dim] containing word embeddings
        """
        # Default GloVe path - update this to your actual GloVe file location
        if glove_path is None:
            glove_path = "./glove/glove.6B.300d.txt"
        
        # Create word to index mapping
        word_to_idx = {word.lower(): i for i, word in enumerate(words)}
        
        # Initialize embeddings with random values
        embeddings = torch.randn(len(words), dim)
        
        # Count how many words we found embeddings for
        found_count = 0
        
        # Try to load pre-downloaded GloVe embeddings
        try:
            with open(glove_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    # Load the full 300d vector first
                        vector = torch.tensor([float(val) for val in values[1:]])
                        
                        # Resize to 256d (using dimensionality reduction)
                        resized_vector = vector[:dim]  # Simple truncation approach
                        
                        idx = word_to_idx[word.lower()]
                        embeddings[idx] = resized_vector
                        found_count += 1
        except FileNotFoundError:
            print(f"GloVe file not found at {glove_path}. Using random embeddings.")
        
        print(f"Found GloVe embeddings for {found_count}/{len(words)} words")
