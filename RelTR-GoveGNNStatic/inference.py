...
def main(args):
    ...
    IdxTocls = dict()
    count = 0
    with open('/raid/home/kirtil/Fatigue_Study/Delsys Processed Data/Outliers/RelTR-git/objects.txt','r')as f:
        while True:
            line = f.readline()
            if not line :
                break
    ...
    print("Length of Relation Classes: ", len(REL_CLASSES))
    ...
    with torch.no_grad():
        ...
        for idx, ax_i, (sxmin, symin, sxmax, symax), (oxmin, oymin, oxmax, oymax) in \
            ...
            ax.axis('off')
            ax.set_title(IdxTocls[probas_sub[idx].argmax()]+' '+REL_CLASSES[probas[idx].argmax()]+' '+IdxTocls[probas_obj[idx].argmax()], fontsize=10)

        fig.tight_layout()
        print("Attention Maps Generated")
        newName = str(args.img_path.split('/')[-1].split('.')[0])
        plt.savefig('./output'+newName+'.png')
        plt.show()
