...
import os
import wandb
wandb.init(project="RelTR-Sentence")
...
def main(args):
    ...
    print(device)
    # print(model.device)
    # print(model.embeddings.device)
    # model.to(device)
    model.knn_indices = model.knn_indices.to(device)
    model.indices_embeddings = model.indices_embeddings.to(device)
    model.HierarchicalTransformerEncoder = model.HierarchicalTransformerEncoder.to(device)
    print("IDX Embedding",model.indices_embeddings.device)
    for name, param in model.named_parameters():
        # print(f"Parameter {name} is on device: {param.device}")
        if(param.device != device):
            print("ERROR: Parameter is not on the correct device")
            print(name)
