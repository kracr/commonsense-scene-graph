...
from .hierarchical_transformer import HierarchicalTransformerEncoder
from .memory_efficient_transformer import CompactEmbeddingEncoder
...
class RelTR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_rel_classes, num_entities, num_triplets, aux_loss=False, matcher=None):
        ...
        # self.alpha_1 = nn.Linear(52,52)
        # self.alpha_2 = nn.Linear(128,52)
        # self.alpha = nn.Linear(104, 52)
        self.cs_rel_pred = MLP(512, 256, num_rel_classes + 1, 2)
    # Fusion MLP takes a concatenation of [visual_relation_logits, cs_relation_logits]
        fusion_in_dim = 2 * (num_rel_classes + 1)
    # We choose an intermediate hidden size here (e.g. 128) but it can be tuned.
        self.fusion_mlp = MLP(fusion_in_dim, 256, num_rel_classes + 1, 2)
        self.HierarchicalTransformerEncoder = HierarchicalTransformerEncoder(
    input_dim=1024,
    hidden_dim=512,
    num_encoder_layers=1,
    num_heads=8,
    dropout=0.1,
    window_size=10,  # Process embeddings in windows of 10
    carrier_tokens=4  # 4 summary tokens per window
)

        
    def getKnnEmbeddings(self, idx1, idx2):
        idx = (idx1*150)+idx2
        return self.indices_embeddings[self.knn_indices[idx]]

    def process_embeddings(self, embed_cat):
        # Reshape the tensor
        embed_cat_new = embed_cat.view(-1, 100, 1024)
        total_items = embed_cat_new.size(0)
        ...
        for i in range(0, total_items, batch_size):
            ...
            batch = embed_cat_new[i:end_idx]
            
            batch_result = self.HierarchicalTransformerEncoder(batch)
            
            # Store result (keeping computation graph intact)
            results.append(batch_result)
        
        # Concatenate all results
        embed_compress = torch.cat(results, dim=0)
        
        return embed_compress
    ...
    def forward(self, samples: NestedTensor):
        ...
        rel1_logits = outputs_class_rel[-1]  # Final relation logits from visual branch; shape: [batch, num_queries, num_rel_classes+1]
    
        # Commonsense branch: create alternative relation logits
        # Get predicted subject/object indices
        indices_obj = outputs_class_obj[-1].argmax(dim=-1)  # [batch, num_queries]
        indices_sub = outputs_class_sub[-1].argmax(dim=-1)  # [batch, num_queries]
        embed_cat = self.getKnnEmbeddings(indices_obj.to('cpu'), indices_sub.to('cpu')).to(indices_obj.device)
        embed_cat_new = embed_cat.view(-1, 100, 512)
        embed_compress = self.process_embeddings(embed_cat_new)
        embed_cat_flat = embed_compress.view(-1, 512)
        visual_feats = torch.cat((hs_sub[-1], hs_obj[-1]), dim=-1)  # shape: [batch, num_queries, 2*embed_dim]
        visual_feats_flat = visual_feats.view(-1, 512) # shape: [batch, num_queries, 512]
        # Call your custom attention (query, key, value). Here we use so_masks features as the query.
        attn_output, _ = self.attention(visual_feats_flat, embed_cat_flat, embed_cat_flat, None)
        # Reshape back to [batch, num_queries, 512]
        attn_output = attn_output.view(visual_feats.shape[0], visual_feats.shape[1],512)
        # Now, pass the attention output through the cs relation head to get relation logits.
        cs_rel_logits = self.cs_rel_pred(attn_output)  # shape: [batch, num_queries, num_rel_classes+1]
        # Fusion via the MLP: concatenate the two sets of relation logits
        fused_input = torch.cat((rel1_logits, cs_rel_logits), dim=-1)  # shape: [batch, num_queries, 2*(num_rel_classes+1)]
        fused_rel_logits = self.fusion_mlp(fused_input)  # shape: [batch, num_queries, num_rel_classes+1]
        torch.cuda.empty_cache()
