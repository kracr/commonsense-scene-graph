...
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, args):
    ...
    if args.dataset == 'vg':
        ...
        zero_shot_evaluator = BasicSceneGraphEvaluator.all_modes(multiple_preds=False)
        zero_shot_image_ids = set()
        with open('./data/vg/zs_test_filenames.txt','r') as f:
            zs_test_filenames = f.readlines()
            for i in range(len(zs_test_filenames)):
                zs_name = int(zs_test_filenames[i][:-8])
                zero_shot_image_ids.add(zs_name)
        if args.eval:
            evaluator_list = []
    ...
    for samples, targets in metric_logger.log_every(data_loader, 100, header):
        ...
        if args.dataset == 'vg':
            evaluate_rel_batch(outputs, targets, evaluator, evaluator_list, zero_shot_evaluator, zero_shot_image_ids)
        else:
    ...
    if args.dataset == 'vg':
        ...
        print('Zero Shot')
        zero_shot_evaluator['sgdet'].print_stats()
...
def evaluate_rel_batch(outputs, targets, evaluator, evaluator_list, zero_shot_evaluator=None, zero_shot_image_ids=None):
    for batch, target in enumerate(targets):
        ...
        if evaluator_list is not None:
            for pred_id, _, evaluator_rel in evaluator_list:
                ...
                if zero_shot_evaluator is not None and image_id in zero_shot_image_ids:
                    # print('Zero Shot Just Worked')
                    zero_shot_evaluator['sgdet'].evaluate_scene_graph_entry(gt_entry, pred_entry)
